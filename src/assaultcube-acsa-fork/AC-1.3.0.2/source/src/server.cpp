...
static bool split_once(const std::string& s, char delim, std::string& left, std::string& right)
{
...
build_uid_to_username_map()
{
    std::unordered_map<std::string, std::string> map; // uidHex -> username
    map.reserve(cli_user_count);

    for (int i = 0; i < cli_user_count; ++i)
    {
        std::string entry = cli_user_ids[i]; // "Name:Secret" (your format)

        std::string username, secret;
        if (!split_once(entry, ':', username, secret)) continue;
        if (username.empty() || secret.empty()) continue;

        uppercase_inplace(secret);

        // recompute exactly how the client created persistent_uid:
        std::string enc = xor_encrypt(username, secret);
        std::string uidHex = to_hex(enc);
...
void export_match_results()
{
    // --- Basic game info ---
    std::string map = sg ? sg->smapname : "unknown";
    std::string mode = modestr ? modestr(sg->smode) : std::to_string(sg->smode);
    int length_ms = sg ? sg->gamemillis : 0;

    // --- Summarize team scores ---
    struct TeamScore { int players = 0, frags = 0, flags = 0; };
    TeamScore teams[2] = {};
    ...
    {
        client* c = clients[i];
        if (!c || c->type == ST_EMPTY || !c->name[0]) continue;

        int t = team_base(c->team);
        teams[t].players++;
        teams[t].frags += c->state.frags;
        if (m_flags_) teams[t].flags += c->state.flagscore;
    }

    // --- Build JSON ---
    std::ostringstream json;
    json << "{\n";
    json << "\"lobbyId\":" << cli_lobby_id << ",\n";
    json << "\"map\":\"" << map << "\",\n";
    json << "\"mode\":\"" << mode << "\",\n";
    json << "\"length_ms\":" << length_ms << ",\n";

    // include userids here
    json << "\"userids\":[";
    for (int i = 0; i < cli_user_count; ++i)
    {
        if (i) json << ",";
    ...
    auto uidToUser = build_uid_to_username_map();
    ...
    loopv(clients)
    {
        ...
        const char* uid = (c->persistent_uid[0] ? c->persistent_uid : "");

        json << "\n{\n"
            << "\"persistent_uid\":\"" << uid << "\",\n"
            << "\"name\":\"" << c->name << "\",\n"
            << "\"team\":" << team_base(c->team) << ",\n"
            << "\"kills\":" << c->state.frags << ",\n"
            << "\"deaths\":" << c->state.deaths;
        if (m_flags_) json << ",\n\"flags\":" << c->state.flagscore;
    ...
    bool ok = NetUtil::http_post_json_bool(
        "https://ac-secure-arena.com",
        L"/api/link/endgameresults",
        json.str(),
        &http_status,
        &body
    );
    ...
    if (ok && http_status >= 200 && http_status < 300)
        mlog(ACLOG_INFO, "Match results uploaded successfully (HTTP %ld)", http_status);
    else
        mlog(ACLOG_ERROR, "Failed to upload match results (ok=%d HTTP %ld body=%s)", ok ? 1 : 0, http_status, body.c_str());
}
#endif
...
static bool g_waitingForPlayers = false;

static void BroadcastPause(bool paused)
{
    sg->sispaused = paused ? 1 : 0;
    sendf(-1, 1, "rii", SV_PAUSEMODE, sg->sispaused);
}

// Put everyone into spectator + kill them (server authority)
static void ForceAllSpectator()
{
    loopv(clients)
    {
        client& cc = *clients[i];
        if (cc.type != ST_TCPIP) continue;

        cc.team = TEAM_SPECT;

        sendf(-1, 1, "ri3", SV_SETTEAM, cc.clientnum, (TEAM_SPECT | (FTR_INFO << 4)));

        // Force death so nobody is alive in-world
        sendf(-1, 1, "ri2x", SV_FORCEDEATH, cc.clientnum, cc.clientnum);
    }
}

// Simple team assign when match becomes ready (2 teams alternating).
// Call your actual spawn helper inside here (tryspawn/sendspawn/etc).
static void AssignTeamsAndSpawn()
{
    int t = 0;
    ...
    loopv(clients)
    {
        ...
        if (cc.team == TEAM_SPECT)
        {
            cc.team = (t++ & 1) ? TEAM_CLA : TEAM_RVSF;
            sendf(-1, 1, "ri3", SV_SETTEAM, cc.clientnum, (cc.team | (FTR_INFO << 4)));

            // ---- SPAWN ----
            // TODO: call your fork’s spawn helper here:
            // tryspawn(&cc);  OR  sendspawn(&cc);
        }
    }
}

// Call this whenever player count might change (join/disconnect).
static void UpdateWaitingGate()
{
    const int players = numnonlocalclients(); // only real remote players
    const bool shouldWait = (players < 2);

    if (shouldWait == g_waitingForPlayers) return;
    g_waitingForPlayers = shouldWait;
    ...
    if (g_waitingForPlayers)
    {
        ...
        defformatstring(msg)("Waiting for players... (%d/2)", players);
        sendf(-1, 1, "ris", SV_TEXT, msg);
    }
    else
    {
        BroadcastPause(false);

        sendf(-1, 1, "ris", SV_TEXT, "Match ready! Spawning players...");
...
{
#ifdef STANDALONE
    // Always evaluate the gate (do NOT early return; welcome must complete)
    UpdateWaitingGate();

    // Always ensure a map exists so clients fully initialize.
    // Gate logic (pause/spec) prevents play until enough players.
    if (!sg->smapname[0] && cli_forced_map[0] && cli_forced_mode >= 0)
    {
        startgame(cli_forced_map, cli_forced_mode, 1, true); // game length 1? (your existing arg)
    }

    // While waiting, keep paused (will be reflected in SV_PAUSEMODE below)
    if (g_waitingForPlayers)
    {
        sg->sispaused = 1;
    }
    ...
    {
#ifdef STANDALONE
        // While waiting, force joiner spectator (server-authoritative)
        if (g_waitingForPlayers)
        {
            c->team = TEAM_SPECT;
        }
...
#ifdef STANDALONE
    // Ensure joiner sees the waiting message immediately too (optional UX)
    if (g_waitingForPlayers)
    {
        defformatstring(msg)("Waiting for players... (%d/2)", numnonlocalclients());
        putint(p, SV_TEXT);
        sendstring(msg, p);
    }
#endif
...
void process(ENetPacket *packet, int sender, int chan)
{
    ...
    while((curmsg = p.length()) < p.maxlen)
    {
        ...
        #ifdef _DEBUG
        if(type!=SV_POS && type!=SV_POSC && type!=SV_POSC2 && type!=SV_POSC3 && type!=SV_POSC4 && type!=SV_CLIENTPING && type!=SV_PING && type!=SV_CLIENT && type != SV_PERSISTENTUID)
        {
            DEBUGVAR(cl->name);
        ...
        switch(type)
        {
            case SV_PERSISTENTUID:
                if (!valid_client(sender)) break;
                char uid[MAXSTRLEN];
                getstring(uid, p);
                copystring(clients[sender]->persistent_uid, uid);
                break;
...
void serverslice(uint timeout)   // main server update, called from cube main loop in sp, or dedicated server loop
{
    ...
    else if(sg->interm && ( (scl.demo_interm) ? sg->gamemillis > (sg->interm<<1) : sg->gamemillis > sg->interm ) )
    {
        ...
        if (!sg->results_sent)
        {
            sg->results_sent = true;
#ifdef STANDALONE
            export_match_results();
#endif
        }

        sendservmsg("Match ended — server shutting down.");

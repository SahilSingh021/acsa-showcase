...
#include <string>
#ifdef STANDALONE
static string cli_forced_map = "";
static int cli_forced_mode = -1;
static std::string cli_user_ids[10];
static int cli_user_count = 0;
static int cli_lobby_id = 0;
#endif // !STANDALONE
    ...
    bool checkarg(const char *arg)
    {
        ...
        { // todo: gjlqEJQU
            ...
            case '-':
                ...
                {
                    copystring(cli_forced_map, arg + 6);
                }
                else if (!strncmp(arg, "--mode=", 7))
                {
                    cli_forced_mode = max(0, atoi(arg + 7));
                }
                else if (!strncmp(arg, "--userids=", 10))
                {
                    const char* ids_str = arg + 10;
                    cli_user_count = 0;

                    const char* start = ids_str;
                    while (*start && cli_user_count < 10)
                    {
                        const char* comma = strchr(start, ',');
                        if (comma)
                        {
                            cli_user_ids[cli_user_count++] = std::string(start, comma - start);
                            start = comma + 1;
                        }
                        else
                        {
                            cli_user_ids[cli_user_count++] = std::string(start);
                            break;
                        }
                    }
                }
                else if (!strncmp(arg, "--lobbyId=", 10))
                {
                    cli_lobby_id = atoi(arg + 10);
                }
#endif // !STANDALONE
                else return false;
                break;

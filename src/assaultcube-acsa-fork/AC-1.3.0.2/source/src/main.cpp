...
int g_HeartbeatThreshold = 0;

char* load_uid_cfg()
{
    static char uid[256] = { 0 };

    using acsa_getpersistentuid = const char* (__cdecl*)();

    HMODULE hMod = GetModuleHandleW(L"ac-anti-tamper.dll");
    if (!hMod) return nullptr;

    auto fn = reinterpret_cast<acsa_getpersistentuid>(GetProcAddress(hMod, "ACSA_GetPersistentUid"));
    if (!fn) return nullptr;

    const char* p = fn();
    if (!p || !*p) return nullptr;

    strncpy(uid, p, sizeof(uid) - 1);
    uid[sizeof(uid) - 1] = '\0';
    return uid;
}

void loadAntiTamper() {
    const char* dllName = "ac-anti-tamper.dll";
    
    HMODULE hMod = LoadLibraryA(dllName);

    if (!hMod)
    {
        MessageBoxA(NULL, "Anti Temper failed to load! Exiting...", "AntiTemper Error", MB_OKCANCEL);
        ExitProcess(1);
    }
}

void setRandomHeartbeatThreshold()
{
    ...
    // Generate random number between 7000 and 15000
    g_HeartbeatThreshold = 7000 + std::rand() % (15000 - 7000 + 1);
}

void antiTamperHeartbeat()
{
    if (g_HeartbeatThreshold == 0)
    {
        setRandomHeartbeatThreshold();
    }

    if (g_HeartbeatCount >= g_HeartbeatThreshold)
    {
        g_HeartbeatCount = 0;
        setRandomHeartbeatThreshold();

        typedef bool (*ac_anti_temper_heartbeat)();
        HMODULE hMod = GetModuleHandleW(L"ac-anti-tamper.dll");

        if (!hMod)
        {
            MessageBoxA(NULL, "Anti Tamper not running! Exiting...", "AntiTamper Error", MB_OK);
            ExitProcess(1);
        }

        auto fn = reinterpret_cast<ac_anti_temper_heartbeat>(GetProcAddress(hMod, "ac_anti_tamper_heartbeat"));
        ...
        {
            MessageBoxA(NULL, "Anti Tamper Heartbeat Fail. Exiting...", "AntiTamper Error", MB_OK);
            ExitProcess(1);
        }
    }
    else
...
int main(int argc, char **argv)
{
    ...
    strcpy(player1->persistent_uid, uid);
